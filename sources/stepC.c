...
{
	S_set S;
	long long unsigned int taille_0, taille_Q;				//taille_0 = le nombre de h_i = 0, taille_Q = le nombre de Q_j "dans" les h_j
	long long int* indice = (long long int*)malloc(F.nb_Q*sizeof(long long int)); //Tableau pour stocker l'indice des vecteurs qui fourniront un carré, il y en a AU MIEUX le nombre de Q_n qu'on a su factoriser
	...
	taille_0 = 0;
//printf("Le nombre de nQ  = %llu\n", F.nb_Q);
	for(long long int i = 0; i < F.nb_Q; i++)	//On cherche les vecteurs mpz_t nuls, i.e. les produits des Q_n B-friable qui forment un carré
	{
	...
	if(taille_0 == 0)
	{
		...
		for(long long int i = 0; i < taille_0; i++)
		{
			...
			//Calcul du Qs associé à As
			taille_Q = 0;

			for(long long int j = F.nb_Q; j > -1; j--)	//On commence à remplir le tableau des Q_j associés aux h_i
			{
				if(mpz_tstbit(F.h[indice[i]],j) == 1)
				{
...
//printf("Étape %lli taille_Q = %llu\n", i, taille_Q);
			//Début du Square Root Procedure
			//Étape 1
			mpz_set(R, tempQ[0]); 
			mpz_set_ui(Qs[i], 1);

			for(long long int j = 1; j < taille_Q; j++)
			{
				//Étape 2
//printf("mouchard étape j = %lld\n", j);	
				mpz_gcd(X, R, tempQ[j]);
...
void isFactored(const mpz_t N, S_set S, mpz_t res)	//Fonction qui renvoie le facteur si on en trouve un ou -1 si on en a pas trouvé
{
	...
	for(long long int i = 0; i < S.taille; i++)
	{
		...
		if(mpz_cmp(sqrA, sqrQ) == 0)
			...
			if(mpz_cmp_ui(PGCD, 1)==0)	//Si le candidat est premier avec N, alors on a pas de facteur et on continue
				continue;
			else{				//On a trouvé une paire qui marche alors on sort de la boucle
				success++;	
				break;
			}
		}
	...
	if(success == 0)			//On a pas trouvé de bon candidat, alors on renvoie -1 pour générer une erreur
		mpz_set_si(res, -1);
	else
		mpz_set(res, PGCD);
